...
// all browsers. Correct the category-id false positive while retaining every other validation.
if(typeof stageMasterImport==='function'&&!stageMasterImport.__v429runtime){const prior=stageMasterImport,wrapped=function(s,kind,rows){const out=prior(s,kind,rows);if(kind==='categories')for(let i=0;i<out.length;i++){if(String(rows?.[i]?.category_id||'').trim()){out[i].errors=(out[i].errors||[]).filter(x=>x!=='missing stable ID');out[i].action=out[i].errors.length?'INVALID':out[i].old?'UPDATE':'ADD'}}return out};wrapped.__v429runtime=true;stageMasterImport=wrapped;globalThis.stageMasterImport=wrapped}
